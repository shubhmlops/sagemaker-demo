...
Pipeline steps (run sequentially via depends_on):
  1. PreprocessData  → 1_preprocessing.py  (SKLearn ProcessingStep)
  2. TrainModel      → 2_training.py        (SKLearn TrainingStep)
  3. EvaluateModel   → 3_evaluation.py      (SKLearn ProcessingStep)
...
def get_pipeline(env: str, region: str):
    ...
    data_key  = os.environ.get("INPUT_DATA_KEY", "sm-pipeline-demo/data/data.csv")
    ...
    step_preprocess = ProcessingStep(
        ...
        code=os.path.join(BASE_DIR, "1_preprocessing.py"),
    )

    # ── STEP 2 — Training ─────────────────────────────────────────────────────
    estimator = SKLearn(
        entry_point=os.path.join(BASE_DIR, "2_training.py"),
        framework_version="1.2-1",
    ...
    step_evaluate = ProcessingStep(
        ...
        code=os.path.join(BASE_DIR, "3_evaluation.py"),
